            ...
            {dataAvailable && (
              <ResponsiveContainer width={"100%"} height={300}>
                <BarChart height={300} data={historyDataQuery.data} barCategoryGap={5}>
                  ...
                  <Tooltip cursor={{ opacity: 0.1 }} content={(props) => <CustomToolTip formatter={formatter} {...props} />} />
                </BarChart>
              </ResponsiveContainer>
            )}
            {!dataAvailable && (
...
function TooltipRow({ label, value, textColor, bgColor, formatter }: { label: string; textColor: string; bgColor: string; value: number; formatter: Intl.NumberFormat }) {
    ...
    <div className="flex items-center gap-2">
      ...
      <div className=" flex w-full justify-between">
        ...
        <div className={cn("text-sm font-bold", textColor)}>
          <CountUp duration={0.5} preserveValue end={value} decimals={0} formattingFn={formattingFn} className="tex-sm" />
        </div>
      </div>
    </div>
  );
